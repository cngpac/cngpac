...
export async function previewCommand(): Promise<void> {
  ...
  if (allChangenotes.length === 0) {
    ...
    p.log.info("Run `cngpac change` to add a changenote first.");
  ...
  const tmpPath = join(tmpdir(), "cngpac-changelog-preview.html");
  writeFileSync(tmpPath, html, "utf-8");

  p.log.success(`Saved preview to ${tmpPath}`);
...
<body>
  <div class="container">
    ...
    <p class="footer">Generated by cngpac</p>
  </div>
</body>
</html>`;
}
