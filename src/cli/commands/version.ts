...
export async function versionCommand(
  ...
  if (!prepareConfig) {
    ...
    p.log.info("Run `cngpac prepare <release|prerelease> [tag]` first.");
  ...
  // Run pre-stage plugins before staging dirty files
  if (config.preStage?.length) {
    for (const plugin of config.preStage) {
      const extraPaths = await plugin({
        versionBump,
        config,
        configDir: rootDir,
      });
      if (extraPaths) {
        const paths = Array.isArray(extraPaths) ? extraPaths : [extraPaths];
        dirtyFiles.push(...paths);
      }
    }
    p.log.success("Pre-stage plugins ran");
  }
